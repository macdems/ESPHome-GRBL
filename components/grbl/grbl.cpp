...
Grbl::Client::Client(AsyncClient* client, std::vector<uint8_t>& recv_buf)
    : tcp_client{client}, identifier{this->format_ip()}, connected{false} {
    ESP_LOGD(TAG, "New client connected from %s", this->identifier.c_str());

    this->tcp_client->onConnect([this](void* h, AsyncClient* client) { this->connected = true; });
...
void Grbl::send_jog_command(float dist_x, float dist_y, float dist_z, int speed) {
    ESP_LOGD(TAG, "Sending jog command: dist_x=%.3f, dist_y=%.3f, dist_z=%.3f, speed=%d", dist_x, dist_y, dist_z, speed);
